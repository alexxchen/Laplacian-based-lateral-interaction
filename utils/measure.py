...
def purity(purity_matrix):
    '''
    For M_{i,j} means the number of examples in cluster i belong to ture class j.
    For purity = m_i * P_i/m, and P_i = max(m_{i,j} / m_i), this make purity = max(m_{i,j} in j) / m
    '''
    purity_max = np.max(purity_matrix, 1)
    _purity = np.sum(purity_max) / np.sum(purity_matrix)
    return _purity
...
class Confusion(object):
    ...
    """
    def __init__(self, k, device, normalized = False):
